...
import { authService, AuthRequest, LoginRequest, RegisterRequest, SocialLoginRequest } from '../auth/auth.js';
...
router.post('/register', async (req: Request, res: Response) => {
  try {
    const { email, password, name, country, timezone }: RegisterRequest = req.body;
...
router.post('/social-login', async (req: Request, res: Response) => {
  try {
    const { provider, providerId, email, name, profilePicture }: SocialLoginRequest = req.body;
...
router.get('/profile', authService.authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const user = req.user!;
...
// Update user profile
router.put('/profile', authService.authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await database.run(
      ...
      [name, country || 'US', timezone || 'UTC', userId]
    );
...
// Change password
router.put('/change-password', authService.authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
// Delete account
router.delete('/account', authService.authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.get('/verify', authService.authenticateToken, async (req: AuthRequest, res: Response) => {
  res.json({
    valid: true,
    user: {
      id: req.user!.id,
