...
router.get('/events', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { start, end, category } = req.query;
        let events;
        if (start && end) {
            events = await database.getEventsByUser(userId, start, end);
        }
        else {
...
router.post('/events', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { title, description, start, end, timezone, allDay = false, location, type = 'personal' } = req.body;
...
router.put('/events/:id', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const eventId = parseInt(req.params.id);
        const { title, description, start, end, timezone, allDay, location, type } = req.body;
        // Validate event ID
        if (isNaN(eventId)) {
            return res.status(400).json({
...
router.delete('/events/:id', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const eventId = parseInt(req.params.id);
        // Validate event ID
...
router.get('/holidays/:country', authService.authenticateToken, async (req, res) => {
    try {
...
router.post('/holidays/:country/sync', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { country } = req.params;
        const { year } = req.body;
