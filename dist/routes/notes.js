...
router.get('/', authService.authenticateToken, async (req, res) => {
    try {
        ...
        let notes;
        if (category) {
            notes = await database.getNotesByUser(userId, category);
        }
        else {
...
router.get('/:id', authService.authenticateToken, async (req, res) => {
    try {
        ...
        if (note.password_hash) {
            ...
            // Verify password
            const isValidPassword = await bcrypt.compare(password, note.password_hash);
            if (!isValidPassword) {
                return res.status(401).json({
...
router.post('/', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { title, content, category = 'general', password, isEncrypted = false } = req.body;
...
// Update note
router.put('/:id', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const noteId = parseInt(req.params.id || '0');
        const { title, content, category, password, currentPassword, isEncrypted } = req.body;
        // Validate note ID
        if (isNaN(noteId)) {
            return res.status(400).json({
                error: 'Invalid note ID'
...
// Get note categories
router.get('/categories/list', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const notes = await database.getNotesByUser(userId);
...
// Search notes
router.get('/search/:query', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const searchResults = notes.filter(note => note.title.toLowerCase().includes(query) ||
...
router.get('/export/:format', authService.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
