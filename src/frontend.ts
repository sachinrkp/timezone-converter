...
// State management
class AppState {
  private _allZones: string[] = [];
  private _inputUtcMs: number | null = null;
  private _diffMinutesAtInput: number = 0;
  private _fromZoneGlobal: string = '';
  private _toZoneGlobal: string = '';
...
// DOM Elements
const elements = {
  sourceAutocomplete: document.getElementById('sourceAutocomplete') as HTMLInputElement,
  targetAutocomplete: document.getElementById('targetAutocomplete') as HTMLInputElement,
  sourceDropdown: document.getElementById('sourceDropdown') as HTMLDivElement,
  targetDropdown: document.getElementById('targetDropdown') as HTMLDivElement,
  form: document.getElementById('converterForm') as HTMLFormElement,
  result: document.getElementById('result') as HTMLDivElement,
  currentTimes: document.getElementById('currentTimes') as HTMLDivElement,
  dstStatus: document.getElementById('dstStatus') as HTMLDivElement,
  sourceSlider: document.getElementById('sourceSlider') as HTMLInputElement,
  targetSlider: document.getElementById('targetSlider') as HTMLInputElement,
  sourceTimeLabel: document.getElementById('sourceTimeLabel') as HTMLDivElement,
  targetTimeLabel: document.getElementById('targetTimeLabel') as HTMLDivElement,
  hourDifference: document.getElementById('hourDifference') as HTMLDivElement,
  timeDifference: document.getElementById('timeDifference') as HTMLDivElement,
  darkToggle: document.getElementById('darkToggle') as HTMLButtonElement,
  swapZones: document.getElementById('swapZones') as HTMLButtonElement,
...
// Utility functions
const utils = {
  two: (n: number): string => n.toString().padStart(2, '0'),
  ...
  cityTimezoneCache: new Map<string, string>(),
  
  // Load city-to-timezone mappings from file
  async loadCityTimezoneMappings(): Promise<void> {
    try {
...
class AutocompleteHandler {
  ...
  private cityMappings: Map<string, string> = new Map();
  ...
  private handleInput(): void {
    ...
    const timezoneFiltered = this.allOptions.filter(option =>
  ...
  private getCityName(timezone: string): string {
...
const api = {
  ...
  async fetchCurrentTime(): Promise<{utc: string, local: string, timestamp: number}> {
    try {
  ...
  async convertTime(
    ...
    date: string,
    time: string
  ): Promise<ConvertTimeResponse | null> {
    try {
      ...
      const data: ConvertTimeResponse = await response.json();
      return data;
    } catch (error) {
...
const ageCalculator = {
  calculateAge: (birthDate: string): { years: number; months: number; days: number; totalDays: number } => {
  ...
  formatAge: (age: { years: number; months: number; days: number; totalDays: number }): string => {
    const parts = [];
...
const dateDifferenceCalculator = {
  calculateDifference: (startDate: string, endDate: string): { years: number; months: number; days: number; totalDays: number } => {
  ...
  formatDifference: (diff: { years: number; months: number; days: number; totalDays: number }): string => {
    const parts = [];
...
const currencyConverter = {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.currencies = lines
          ...
          .filter(currency => currency !== null) as CurrencyData[];
  ...
  convert: (amount: number, fromCurrency: string, toCurrency: string): { convertedAmount: number; rate: number } => {
...
const ui = {
  updateResults: (data: ConvertTimeResponse): void => {
    ...
    if (elements.dstStatus) {
      const dstActiveNow = utils.tzOffsetMinutes(appState.inputUtcMs!, appState.toZoneGlobal) !==
        Math.max(
          utils.tzOffsetMinutes(Date.UTC(new Date().getFullYear(), 0, 1, 12, 0, 0), appState.toZoneGlobal),
          utils.tzOffsetMinutes(Date.UTC(new Date().getFullYear(), 6, 1, 12, 0, 0), appState.toZoneGlobal)
        );
  ...
  initSliders: (): void => {
    if (!elements.sourceSlider || !elements.targetSlider) return;
  ...
  updateTimelineForTimezones: (): void => {
    if (!appState.fromZoneGlobal || !appState.toZoneGlobal) return;
  ...
  updateSliderLabels: (srcOffsetMinutes: number, tgtOffsetMinutes: number): void => {
    if (!elements.sourceTimeLabel || !elements.targetTimeLabel) return;
...
const setupEventListeners = (): void => {
  ...
  const humanToEpochForm = document.getElementById('humanToEpochForm') as HTMLFormElement;
  const epochToHumanForm = document.getElementById('epochToHumanForm') as HTMLFormElement;

  if (humanToEpochForm) {
    humanToEpochForm.addEventListener('submit', (e) => {
      e.preventDefault();
      
      const date = (document.getElementById('humanDate') as HTMLInputElement).value;
      const time = (document.getElementById('humanTime') as HTMLInputElement).value;
  ...
  if (epochToHumanForm) {
    epochToHumanForm.addEventListener('submit', (e) => {
      ...
      const epochInput = (document.getElementById('epochInput') as HTMLInputElement).value;
  ...
  // Age Calculator form
  const ageCalculatorForm = document.getElementById('ageCalculatorForm') as HTMLFormElement;
  ...
  const dateDifferenceForm = document.getElementById('dateDifferenceForm') as HTMLFormElement;
  ...
  const dateArithmeticForm = document.getElementById('dateArithmeticForm') as HTMLFormElement;
  if (dateArithmeticForm) {
    dateArithmeticForm.addEventListener('submit', (e) => {
      e.preventDefault();
      
      const baseDate = (document.getElementById('baseDate') as HTMLInputElement).value;
      const years = parseInt((document.getElementById('yearsInput') as HTMLInputElement).value) || 0;
      const months = parseInt((document.getElementById('monthsInput') as HTMLInputElement).value) || 0;
      const days = parseInt((document.getElementById('daysInput') as HTMLInputElement).value) || 0;
  ...
  const currencyConverterForm = document.getElementById('currencyConverterForm') as HTMLFormElement;
...
const initApp = async (): Promise<void> => {
  try {
    ...
    const epochTimezoneInput = document.getElementById('epochTimezone') as HTMLInputElement;
    const humanTimezoneDropdown = document.getElementById('humanTimezoneDropdown') as HTMLDivElement;
    const epochTimezoneDropdown = document.getElementById('epochTimezoneDropdown') as HTMLDivElement;
